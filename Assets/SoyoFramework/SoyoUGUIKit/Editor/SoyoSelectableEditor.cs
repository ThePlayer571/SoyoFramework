...
using SoyoFramework.SoyoUGUIKit.Runtime;
using UnityEditor;
using UnityEditor.UI;
using UnityEngine;
using UnityEngine.UI;

namespace SoyoFramework.SoyoUGUIKit.Editor
{
    [CustomEditor(typeof(SoyoSelectable), true)]
    public class SoyoSelectableEditor : SelectableEditor
    {
        public override void OnInspectorGUI()
        {
            ...
            // 只显示实际声明在本类及其子类的字段
            DrawCustomFields(so, type, typeof(Selectable));
        }
        ...
        private void DrawScriptField()
        {
            ...
            {
                // 注意：CustomEditor(typeof(SoyoSelectable), true) 允许子类也用这个 Editor
                // 这里用 target.GetType() 来拿到实际运行时类型对应的脚本
